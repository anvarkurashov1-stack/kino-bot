...
# ==========================================
# RENDER UCHUN KICHIK WEB SERVER (PORT OCHISH)
# ==========================================
app = Flask(__name__)
...
def get_movie_by_code(code):
  ...
  cursor = conn.cursor()
  cursor.execute(
      "SELECT title, caption, file_id, genre FROM movies WHERE code = ?",
      (code,),
  )
  row = cursor.fetchone()
  conn.close()
...
def find_movie(message):
  ...
  if movie:
    ...
    bot.send_message(
        message.chat.id,
        "❌ Bunday kodli kino topilmadi. Iltimos, to'g'ri kod kiriting.",
    )
